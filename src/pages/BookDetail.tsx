...
const bookInfoList = [
    {
        label: '카테고리',
        key: 'categoryName',
        filter: (book: IBookDetail) => <Link to={`/books?category_id=${book.categoryId}`}>{book.category}</Link>
    },
    {
        label: '포맷',
        key: 'form'
    ...
    return (
        <BookDetailStyled>
            <header className="header">
                ...
                <div className="info">
                    ...
                    {bookInfoList.map((info) => (
                        <dl key={info.key}>
                            <dt>{info.label}</dt>
                            <dd>{info.filter ? info.filter(book) : book[info.key as keyof typeof book]}</dd>
                        </dl>
                    ))}
                    <p className="summary">
                        {book.summary}
            ...
            <div className="content">
                ...
                <EllipsisBox lines={4}>{book.detail}</EllipsisBox>
...
const BookDetailStyled = styled.div`
